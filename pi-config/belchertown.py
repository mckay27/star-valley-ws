...
class getData(SearchList):
    ...
    def get_cardinal_direction(self, degree, return_only_labels=False):
        ...
        try:
            ...
            try:
                ordinate_names = [
                    unicode(x, "utf-8") for x in ordinate_names
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        if self.generator.skin_dict["Extras"]["belchertown_locale"] == "auto":
            ...
            try:
                ...
                if belchertown_debug:
                    logerr(
                        "Locale: Error using locale %s. This locale may not be installed on your system and you may see unexpected results. Belchertown skin JavaScript will try to use this locale. Full error: %s"
                        % (
                            self.generator.skin_dict["Extras"]["belchertown_locale"],
                            error,
                        )
        ...
        if self.generator.skin_dict["Extras"]["radar_html"] == "":
            ...
            if (
                ...
                marker = ""
            radar_html = '<iframe width="650" height="360" src="https://embed.windy.com/embed2.html?lat={}&lon={}&zoom={}&level=surface&overlay=radar&menu=&message=true&marker={}&calendar=&pressure=&type=map&location=coordinates&detail=&detailLat={}&detailLon={}&metricWind=&metricTemp=&radarRange=-1" frameborder="0"></iframe>'.format(
                lat, lon, zoom, marker, lat, lon
            )
        else:
        ...
        try:
            ...
            years = sorted(set(years))[
                ::-1
            ]  # Remove duplicates with set, and sort numerically, then reverse sort with [::-1] oldest year last
            # first_year = years[0]
            # final_year = years[-1]
        ...
        if (
            ...
            if forecast_provider == "aeris":
                ...
                if self.generator.skin_dict["Extras"]["forecast_alert_limit"]:
                    ...
                    forecast_alerts_url = (
                        "https://api.aerisapi.com/alerts/%s,%s?&format=json&limit=%s&client_id=%s&client_secret=%s"
                        % (
                            latitude,
                            longitude,
                            forecast_alert_limit,
                    ...
                    forecast_alerts_url = (
                        "https://api.aerisapi.com/alerts/%s,%s?&format=json&limit=1&client_id=%s&client_secret=%s"
                        % (latitude, longitude, forecast_api_id, forecast_api_secret)
                    )
                ...
                forecast_url = (
                    "https://api.darksky.net/forecast/%s/%s,%s?units=%s&lang=%s"
                    % (
                        forecast_api_secret,
                        latitude,
                        longitude,
            ...
            if forecast_is_stale:
                try:
                                    ...
                                    forecast_file_result = json.dumps(
                                        {
                                            ...
                                            "forecast": [json.loads(forecast_page)],
                    ...
                    raise Warning(
                        "Error downloading forecast data. Check the URL in your configuration and try again. You are trying to use URL: %s, and the error is: %s"
                        % (forecast_url, error)
                    )
            ...
            if forecast_provider == "aeris":
                ...
                if (data["flags"]["units"].lower() == "us") or (
                    data["flags"]["units"].lower() == "uk2"
                ):
                    visibility_unit = "miles"
                elif (data["flags"]["units"].lower() == "si") or (
                    data["flags"]["units"].lower() == "ca"
                ):
                    visibility_unit = "km"
                else:
                    visibility_unit = ""
        ...
        if self.generator.skin_dict["Extras"]["earthquake_enabled"] == "1":
            ...
            if earthquake_is_stale:
                ...
                try:
                    ...
                    if weewx.debug:
                        logdbg(
                            "Error downloading earthquake data with urllib2, reverting to curl and subprocess. Full error: %s"
                            % forecast_error
                        )
                    ...
                    try:
                        ...
                        raise Warning(
                            "Error downloading earthquake data using urllib2 and subprocess curl. Your software may need to be updated, or the URL is incorrect. You are trying to use URL: %s, and the error is: %s"
            ...
            try:
                if self.generator.skin_dict["Extras"]["earthquake_server"] == "USGS":
                    ...
                    eqtime = int(
                        eqtime.replace(tzinfo=datetime.timezone.utc).timestamp()
                    )
                    equrl = (
        ...
        for obs in station_observations:
            ...
            if obs == "visibility":
                try:
                    ...
                    raise Warning(
                        "Error adding visiblity to station observations table. Check that you have forecast data, or remove visibility from your station_observations Extras option."
                    )
        ...
        for obs in sorted(weewx.units.obs_group_dict):
            ...
            if obs not in all_obs_rounding_json:
                ...
                obs_unit_label = self.generator.skin_dict["Units"]["Labels"].get(
                    obs_unit, ""
                )
        ...
        for chart_group in self.chart_dict.sections:
            ...
            for plotname in self.chart_dict[chart_group].sections:
                ...
                output[chart_group][plotname][
                    ...
                    OrderedDict()
                )  # This retains the observation position in the dictionary to match the order in the conf so the chart is in the right user-defined order
                ...
                for line_name in self.chart_dict[chart_group][plotname].sections:
                    ...
                    if time_length == "today":
                        ...
                        if minstamp is None or maxstamp is None:
                            raise Warning(
                                "Error trying to create timespan_specific graph. You are missing either timespan_start or timespan_stop options."
                            )
                        ...
                        if start_at_midnight:
                            ...
                            )  # Take the generation time and subtract the time_length to get our start time
    ...
    def get_observation_data(
            ...
            for windData in merged:
                if windData[0] is not None and windData[1] is not None:
                    if (
                        ...
                        or windSpeed_unit == "mile_per_hour2"
                    ):
                        if windData[1] < 1:
                        ...
                        windSpeed_unit == "km_per_hour"
                        or windSpeed_unit == "km_per_hour2"
                    ):
                        if windData[1] < 2:
                            group_0_windDir.append(windData[0])
                            group_0_windSpeed.append(windData[1])
                        ...
                        windSpeed_unit == "meter_per_second"
                        or windSpeed_unit == "meter_per_second2"
                    ):
                        if windData[1] < 0.5:
                            group_0_windDir.append(windData[0])
                            group_0_windSpeed.append(windData[1])
        ...
        if observation == "weatherRange":
            ...
            if weatherRange_obs_lookup is not None:
                ...
                raise Warning(
                    "Error trying to create the weather range graph. You are missing the range_type configuration item."
                )
        ...
        #     output_data = zip(time_ms, obs_vt)
        ...
        if observation == "rainTotal":
            ...
            if observation == "barometer":
                ...
                usage_round = int(
                    self.skin_dict["Units"]["StringFormats"].get(obs_vt[2], "2f")[-2]
                )
